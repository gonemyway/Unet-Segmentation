...
# Import thư viện segmentation_models
...
# Tạo tập dữ liệu và load theo từng batch
class Dataset:
...
# Load thông tin từ dataset để tạo ra 2 biến image_path và mask_path
def load_path(data_path):
    # Get normal image and mask
    classes = ['Class1', 'Class2', 'Class3', 'Class4', 'Class5', 'Class6']

    # Lặp qua các thư mục ảnh không lỗi
    normal_image_path = []
    normal_mask_path = []
    ...
    # Drop bớt ảnh sản phẩm không lỗi
...
model = Unet(BACKBONE, encoder_weights="imagenet", classes=1, activation="sigmoid", input_shape=(512, 512, 3), encoder_freeze=True)  # encoder_freeze=True: dùng pretrain của imagenet mà không cần điều chỉnh theo data train
loss1 = sm.losses.categorical_focal_dice_loss
model.compile(optimizer=opt, loss=loss1, metrics=[iou_score])

# Train model
# Lưu lại weight có IOU Score lớn nhất
is_train = False
if is_train:
    ...
    for id in index:
        ...
        # Ảnh sản phẩm, không phải ảnh mask
        image = cv2.imread(image_test[id])
        image = cv2.resize(image, (512, 512))

        # Đưa qua model để predicted segmentation map
